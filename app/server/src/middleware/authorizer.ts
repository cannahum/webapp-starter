...
export const authorizer: AuthChecker<IAppContext> = async ({root, args, context, info}, roles): Promise<boolean> => {
  const {user} = context;
  ...
  const onlyAuthentication: boolean = roles.length === 0;
  // only regular users is also just only authentication.
  const onlyRegularUsers: boolean = roles.length === 1 && roles[0] === String(AuthLevel.REGULAR);
